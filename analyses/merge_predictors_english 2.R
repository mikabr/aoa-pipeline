...
match_morph_to_childes <- function(name_collection, name_corpus){
  ...
  w <- english_uk_word  %>% filter(corpus_name==name_corpus)
  w <- w[order(w$transcript), ] #%>%   ###remove extra sessions
  #filter(transcript != "Champaud/011003.xml" )
  w <- tibble::rowid_to_column(w, "matching_index")
  ...
  language <- base %>%  filter(corpus_name==name_corpus)
...
belfast_morphology <- do.call("rbind", list( final_belfast )) %>%
  select(id, stem, pos, prefix, affix, affix_type, gloss, corpus_name, suffix, utterance_id)

thomas_morphology <- thomas_morphology[-c(517037), ]
...
thomas_morphology_ <- thomas_morphology %>%
...
uk_morphology_2 <- uk_morphology_2 %>%
  group_by(id, utterance_id, corpus_name, gloss) %>%
  summarise(stem = paste(sort(unique(stem)),collapse=", "),
            affix = paste(sort(unique(affix)),collapse=", "),
            affix_type = paste(sort(unique(affix_type)),collapse=", "),
...
saveRDS(tokens_uk_, "~/Documents/morphological_predictors/spanish_uk.rds")
...
english_us_mor <- readRDS('/Users/loukatou/Documents/morphological_predictors/childes-mor-data/childes/Eng-US/mor.rds')
english_us_mk <- readRDS('/Users/loukatou/Documents/morphological_predictors/childes-mor-data/childes/Eng-US/mk.rds')
english_us_word <- readRDS('/Users/loukatou/Documents/morphological_predictors/childes-mor-data/childes/Eng-US/w.rds') %>%
  dplyr::select(-pos) %>%
  mutate(transcript = gsub("/Users/mikabr/childes/childes/Eng-US/", "", transcript)) %>%
  mutate(corpus_name = gsub("/.*", "", transcript)) %>%
  mutate(incomplete_type=ifelse(is.na(incomplete_type), "0", incomplete_type))%>%
  filter(!incomplete_type =="omission")
