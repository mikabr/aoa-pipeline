...
#aoa_predictor_data <- aoas |> left_join(data_lexcat) |> remove_NA_predictors(predictors) |> filter(n_tokens == 1)
...
lang_data1 <- aoa_predictor_data |>
  filter(language=="English (American)" & measure=="produces") |>
  select(language, uni_lemma, definition, word_clean, aoa, lexical_category, avg_surprisal, all_frequency, concreteness)

loo_df <- crossv_loo(lang_data1)

fit_cv_models_single <- function(id) {
  models <- "no model"
  train_idx <- loo_df[id,1][[1]][[1]]$idx
  test_idx <- loo_df[id,2][[1]][[1]]$idx
  train_df <- lang_data[train_idx,]
...
loo_models <- fit_cv_models(lang_data, formulae)
