...
surprisals <- lstm_surprisals |> mutate(lstm_surprisal = avg_surprisal) |> select(-c(n_instances, avg_surprisal))
```

```{r load_ngram_childes_suprisals}
files  <- list.files(path = "./surprisals/ngram/ngram_childes_surprisal/", full.names = TRUE)
tables <- lapply(files, read.csv, header = TRUE)
ngram_childes_surprisal <- do.call(rbind , tables) %>% mutate(avg_surprisal = (surprisal_2gm+surprisal_3gm+surprisal_4gm+surprisal_5gm+surprisal_6gm)/5, all_frequency = frequency, avg_perplexity = NA)
...
tables <- lapply(files, read.csv, header = TRUE)
ngram_wiki_surprisal <- do.call(rbind , tables) %>% mutate(avg_surprisal = (surprisal_2gm+surprisal_3gm+surprisal_4gm+surprisal_5gm+surprisal_6gm)/5, all_frequency = frequency, avg_perplexity = NA)
...
# ngram_wiki
predictor_data_ngram_wiki <- ngram_wiki_surprisal |> left_join(concreteness)
# ngram_childes
predictor_data_ngram_childes <- ngram_childes_surprisal |> left_join(concreteness)
```
...
predictor_sources <- list(
  c("lstm_surprisal", "all_frequency"),
  "concreteness")
...
four_surp = ~ lexical_category * fourgram_surprisal + lexical_category * all_frequency + lexical_category * concreteness
tri_surp = ~ lexical_category * trigram_surprisal + lexical_category * all_frequency + lexical_category * concreteness
bi_surp = ~ lexical_category * bigram_surprisal + lexical_category * all_frequency + lexical_category * concreteness
null_model = ~ 1
...
predictors <- c("lstm_surprisal", "fourgram_surprisal", "trigram_surprisal", "bigram_surprisal", "all_frequency", "concreteness")

scaled_lang_data <- aoa_predictor_data |>
  filter(language==lang & measure==ms) |>
  select(language, uni_lemma, category, definition, word_clean, aoa, lexical_category, lstm_surprisal, fourgram_surprisal, trigram_surprisal, bigra_surprisal, all_frequency, concreteness) |>
