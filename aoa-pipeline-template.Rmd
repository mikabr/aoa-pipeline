...
loo_df <- aoa_predictor_data |>
  ...
  mutate(loo_models = ifelse(language %in% sinotibetan_languages, map(data, fit_cv_models, list(make_predictor_formula(predictors_short))), map(data, fit_cv_models, list(make_predictor_formula(predictors)))),
