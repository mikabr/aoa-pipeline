...
#The output has: 1. raw counts of uni_lemmas (count, count_first, count_last, count_solo) 2. sum count of whole CHILDES corpus (sumcount, sumcount_first, sumcount_last, sumcount_solo) 3. freq_raw is the raw frequency (dividing count / sumcount)
```
...
```{r}
cv_model <- function(preds, loo_df, lang, meas, word_values){
...
cv_main<-function(lang, word_values, meas, lex=FALSE){
  word_values_ <- word_values %>% filter(language==lang, measure==meas) 
  if (length(unique(word_values_$language) ) == 0){break}
  else {if (lex==FALSE){
  m<-map_df(preds, cv_model, crossv_loo(word_values_), lang, meas, word_values_)
    }
    else{
  m<-map_df(preds, cv_model_lex, crossv_loo(word_values_), lang, meas, word_values_)    
    }
  return(m)
  }
} 

cv_across_lang <- map_df(unique(word_values_$language), cv_main, word_values, "produces") %>%
  rbind(map_df(unique(word_values_$language), cv_main, word_values, "understands")) #####cross-validation for each language


cv_across_lang_lex <- map_df(unique(word_values_$language), cv_main, word_values, meas="understands", lex=TRUE) %>%
  rbind(map_df(unique(word_values_$language), cv_main, word_values, "produces", lex=TRUE)) ####check lexical category distribution 

head(arrange(cv_across_lang_lex %>% filter(language=="English (American)", measure=="produces"), desc(abs_dev)), 50) 
```
