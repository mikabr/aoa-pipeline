...
wb_data <- map_df(target_langs, function(lang) {
  ...
  tryCatch( 
    {
      # If data for language X is already cached, it will be loaded directly into the workspace
      readRDS(glue("data/wordbank/{norm_lang}.rds"))
    },
    error = function(e) {
      # If the data for language X is not cached, it will download it for all available instruments types, cache the data for future use and then load it into the workspace
      print(glue("No cached data for {lang}, downloading data now..."))
      create_wb_data(lang)
      readRDS(glue("data/wordbank/{norm_lang}.rds"))
    }
    )
