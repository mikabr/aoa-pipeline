...
Creating saved Wordbank data for uncashed language, example with English
```{r create_wordbank_eng, eval=FALSE}
create_wb_data("English (American)")
```

Creating saved Wordbank data one step at a time (potentially making changes between steps) for uncashed language, example with English :
...
Loading Wordbank data for multiple languages (cashed or not):
```{r loadwordbankxling}
