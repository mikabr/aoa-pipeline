...
abstract: >
    ...
    We studied 10 predictors of word ages of acquisition across 28 languages and dialects, finding that words that were more frequent, concrete, and associated with babies were learnt earlier, whereas words that had greater length in phonemes and mean length of utterance in words (MLU-w) were learnt later. 
    There was no reliable effect of other morphosyntactic predictors, or of phonological neighbourhood size. 
    We found evidence of a tradeoff between the morphological complexity of a language and the effect of MLU-w for predicates. 
...
The VIFs for each predictor in each language was $<3.5$, indicating low multicollinearity among predictors.
...
The average correlation across languages was relatively high ($\bar{r}$ = `r dd_cor_gen |> pull(estimate) |> mean()`), comparable to results found by Braginsky et al. (2019).
...
The coefficient of the effect of morphological complexity was negative ($b$ = `r mc_vals |> filter(ms_term == "MLU-w", lexical_category == "Predicates", term == "morph_complexity") |> pull(estimate)`), suggesting that languages with greater morphological complexity had a smaller effect of MLU-w for the AoA of predicates.
...
We found that these effects were generally consistent across typologically diverse languages, suggesting that early word learning broadly taps on similar sources of linguistic information despite disparities in the realisations of such information across languages. 
...
Morphosyntactic predictors may be more crucial when learning other aspects of morphosyntax, particularly at the form level (e.g., deploying appropriately inflected forms). 
...
The consistency in observed predictor values suggests that such a potential bias was not very large in magnitude, but it remains crucial to increase resource availability in understudied languages to permit more accurate study of these languages.
