...
get_childes_metrics <- function(lang = NULL,
  ...
  args_<-list(lang, corpus, speaker_role, speaker_role_exclude, child_age, child_sex, pos, word)
 
  data_<- do.call(get_data, args_)
  
  if (freq == TRUE){
  wordcount = frequency(data_$tokens)
  metrics <- full_join(metrics, wordcount)
  }
  
  if (uttlength == TRUE){
  mlu = mlu(data_$utterances, data_$tokens, column)
  metrics <- full_join(metrics, mlu)
  }
  
  if (wordlen == TRUE){
  wlength = wordlength( data_$tokens)
  metrics <- full_join(metrics, wlength)
  }
  
  if (order == TRUE){
  worder = order(data_$utterances, data_$tokens, column)
  metrics <- full_join(metrics, worder)
  }
...
group_by_column <- function(df, column){
  df <- df %>%
  group_by_(column) %>%  
    count() 
}
...
tokens1<- group_by_column(tokens_, "corpus_name") %>%
      rename(corpuscount = n)
tokens2<- group_by_column(tokens_, "speaker_role") %>%
      rename(speakercount = n)
tokens3<- group_by_column(tokens_, "target_child_name") %>%
      rename(targetchildcount = n)

tokens_all <- tokens_ %>% left_join(tokens1)  %>% left_join(tokens2)  %>% left_join(tokens3)
...
tokens <-get_tokens(language = lang, corpus = corpus, role = speaker_role, role_exclude = speaker_role_exclude, age = child_age, sex = child_sex, part_of_speech = pos, token = word) %>%
  left_join(tokens_all)}
else {tokens <- tokens_all}

tokens$gloss <-tolower(tokens$gloss)
utterances$gloss <-tolower(utterances$gloss)

tokens <- tokens %>%
  select(id, gloss, language, corpus_name, speaker_role, target_child_name, target_child_age, target_child_sex, utterance_id, corpuscount, speakercount, targetchildcount)
...
adjust_level <- function(tokens){
if (column == "target_child_name"){
tokens$levelname <- tokens$target_child_name
tokens$levelcount<- tokens$targetchildcount
}  else if (column == "speaker_role"){
tokens$levelname <- tokens$speaker_role
tokens$levelcount<- tokens$speakercount
}  else if (column == "corpus_name"){
tokens$levelname<- tokens$corpus_name
tokens$levelcount<- tokens$corpuscount
}  else {
tokens$levelname <- tokens$language
tokens$levelcount<- nrow(tokens) 
}
return(tokens)
}


frequency<-function(tokens, column = "language"){ 
tokens <- adjust_level(tokens)
tokens_grouped <- tokens %>%
    select(levelname, gloss, levelcount)  %>% #corpuscount
      group_by(levelname, gloss) %>%
      count()  %>% 
        rename(wordcount = n) 
tokens_divide <- tokens %>% 
    left_join(tokens_grouped) %>%
      mutate(rawfreq=wordcount/levelcount) %>%
          select(gloss, rawfreq, wordcount, levelcount, levelname)
...
childes_metrics<- get_childes_metrics(lang = "ita", word="", uttlength = TRUE, freq = TRUE, wordlen = TRUE, order = TRUE)
